 ...
 */
userRoutes.post("/add", hasPermissions('system.user.create'), validateData(userAddSchema), async (req, res) => {
 ...
 *           description: The list of role IDs associated with the user
 */
userRoutes.put("/:id/edit", hasPermissions('system.user.edit'), validateData(userEditSchema), async (req, res) => {
  const idParam = req.params.id;
 ...
 *
 */
userRoutes.post("/validate-username", hasPermissions('system.user.edit'), validateData(usernameValidationSchema), async (req, res) => {
